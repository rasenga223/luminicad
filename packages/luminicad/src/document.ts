...
export class Document extends Observable implements IDocument {
    ...
    async save() {
        // Log the ID of the document being saved.
        Logger.info(`Attempting to save document with ID: ${this.id} and name: ${this.name}`);

        const data = this.serialize();
        await this.application.storage.put(Constants.DBName, Constants.DocumentTable, this.id, data);

        if (this.application.activeView) {
            // Log the document ID and name associated with the current activeView before updating and capturing.
            Logger.info(
                `Active view is for document ID: ${this.application.activeView.document.id}, name: ${this.application.activeView.document.name}`,
            );
            this.application.activeView.update();
        } else {
            // Log if there is no active view at the time of saving.
            Logger.warn(`No activeView found when saving document ID: ${this.id}`);
        }

        const image = this.application.activeView?.toImage();

        // Log after image capture, confirming which document's view was supposedly captured.
        if (this.application.activeView) {
            Logger.info(
                `Image captured for document ID: ${this.application.activeView.document.id}, name: ${this.application.activeView.document.name}`,
            );
        } else {
            Logger.warn(`Image capture attempted without an activeView for document ID: ${this.id}`);
        }
        ...
        // Log successful save completion for this document.
        Logger.info(`Successfully saved document ID: ${this.id}, name: ${this.name} to RecentTable.`);
    }
